...
function getPrismaClient(): any {
	...
	try {
		const backendClientPath = path.join(__dirname, '..', 'backend', 'node_modules', '@prisma', 'client')
		// eslint-disable-next-line @typescript-eslint/no-var-requires
		return require(backendClientPath).PrismaClient
	} catch (err) {
		// Next try worker generated client
		try {
			const workerClientPath = path.join(__dirname, '..', 'worker', 'node_modules', '@prisma', 'client')
			// eslint-disable-next-line @typescript-eslint/no-var-requires
...
const globalForPrisma = (globalThis as any) as { prisma?: InstanceType<typeof PrismaClientCtor> }
