...
app.post('/:id/install', async (c) => {
  try {
    ...
    // Preload the node
    await nodeLoader.loadNode(install.node.name, install.version);
...
app.delete('/:id/install', async (c) => {
  try {
    ...
    await customNodeService.uninstallNode(nodeId, organizationId, user.id);
